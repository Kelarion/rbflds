...
class Trainer(object):
    ...
    def train(
        ...
        if 'mark_probs' not in data and model.gen_net.num_clusters is not None:
            raise ValueError('must supply mark_probs for this generative model')
        elif 'mark_probs' not in data:
            data['mark_probs'] = None
...
#        cost_train /= len(train_indxs)
    ...
    def _get_feed_dict(self, model, data=None, batch_indxs=None):
        ...
        if batch_indxs is not None:
            ...
            if data['mark_probs'] is not None:
                feed_dict[model.gen_net.mark_probs] = \
                    data['mark_probs'][batch_indxs, :, :, :]
                feed_dict[model.inf_net.mark_probs] = \
                    data['mark_probs'][batch_indxs, :, :, :]
            ...
            if data['mark_probs'] is not None:
                feed_dict[model.gen_net.mark_probs] = data['mark_probs']
                feed_dict[model.inf_net.mark_probs] = data['mark_probs']
            for indx_, data_ in enumerate(data['linear_predictors']):
